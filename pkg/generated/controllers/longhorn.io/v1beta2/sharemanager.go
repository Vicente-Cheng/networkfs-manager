...
// RegisterShareManagerStatusHandler configures a ShareManagerController to execute a ShareManagerStatusHandler for every events observed.
...
func RegisterShareManagerStatusHandler(ctx context.Context, controller ShareManagerController, condition condition.Cond, name string, handler ShareManagerStatusHandler) {
	statusHandler := &shareManagerStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
...
// RegisterShareManagerGeneratingHandler configures a ShareManagerController to execute a ShareManagerGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
func RegisterShareManagerGeneratingHandler(ctx context.Context, controller ShareManagerController, apply apply.Apply,
	...
	statusHandler := &shareManagerGeneratingHandler{
		ShareManagerGeneratingHandler: handler,
		apply:                         apply,
		name:                          name,
	...
	RegisterShareManagerStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type shareManagerStatusHandler struct {
	client    ShareManagerClient
	condition condition.Cond
	handler   ShareManagerStatusHandler
}

// sync is executed on every resource addition or modification. Executes the configured handlers and sends the updated status to the Kubernetes API
func (a *shareManagerStatusHandler) sync(key string, obj *v1beta2.ShareManager) (*v1beta2.ShareManager, error) {
	if obj == nil {
		return obj, nil
	}
...
type shareManagerGeneratingHandler struct {
	ShareManagerGeneratingHandler
	apply apply.Apply
...
func (a *shareManagerGeneratingHandler) Remove(key string, obj *v1beta2.ShareManager) (*v1beta2.ShareManager, error) {
	if obj != nil {
		return obj, nil
	}

	obj = &v1beta2.ShareManager{}
...
// Handle executes the configured ShareManagerGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
func (a *shareManagerGeneratingHandler) Handle(obj *v1beta2.ShareManager, status v1beta2.ShareManagerStatus) (v1beta2.ShareManagerStatus, error) {
	if !obj.DeletionTimestamp.IsZero() {
		return status, nil
	}

	objs, newStatus, err := a.ShareManagerGeneratingHandler(obj, status)
	if err != nil {
...
func (a *shareManagerGeneratingHandler) isNewResourceVersion(obj *v1beta2.ShareManager) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *shareManagerGeneratingHandler) storeResourceVersion(obj *v1beta2.ShareManager) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
